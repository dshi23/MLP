...
def one_hot_encoding(labels, num_classes=10):
    ...
    one_hot[range(len(labels)), labels.reshape(len(labels))] = 1
    return one_hot
...
def calculateCorrect(y, t):  #Feel free to use this function to return accuracy instead of number of correct predictions
    ...
    num_correct = np.sum(predicted == target)
    return num_correct
...
def append_bias(X):
    ...
    """
    N = X.shape[0]
    X_bias = np.append(np.ones((N,1)),X,1)
    return X_bias
...
def plots(trainEpochLoss, trainEpochAccuracy, valEpochLoss, valEpochAccuracy, earlyStop):
    ...
    plt.savefig(constants.saveLocation+"loss.jpg")
    plt.show()
    ...
    plt.savefig(constants.saveLocation+"accuarcy.jpg")
    plt.show()
...
def createTrainValSplit(x_train,y_train):
    ...
    train_index, val_index = index[:int(0.8*N)], index[int(0.8*N):]
    return x_train[train_index], y_train[train_index], x_train[val_index], y_train[val_index]
